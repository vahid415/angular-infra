...
export class MasterFormComponent implements OnInit {
  ...
  /**
   * pass dynamic detail template for set in grid detail
   */
  ...
  refresh() {
    ...
    this.controller.searchFilter.forEach(x => x.value = null);
    this.pagingReq.start = this.gridOption.start;
    this.pagingReq.size = this.gridOption.pageSize;
  }
   ...
   */
  selectEntityInShowMode(entity: any) {
  ...
  changeToggle(field, obj) {
    this.ui.showConfirm(
      ...
      , () => {
        if (obj[ field ]) {
          this.controller.masterFormService.active(obj.id).subscribe((data) => {
          ...
          this.controller.masterFormService.inActive(obj.id).subscribe((data) => {
            this.successToast(field, obj);
            this.startSearch();
          }, (error) => {
            this.selectedRow[ field ] = !obj[ field ];
