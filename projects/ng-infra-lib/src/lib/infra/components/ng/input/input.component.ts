...
export class InputComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  @Input() placeholder = '';
  @Input() conditions: { key: string, value: any, error?: string }[] = null;
...
// 1: delete all [width], [height], [modal], [positionCenter], [showMin], [showMax], [title] only delete translator pipes which are not necessary now, change (show) && (close) to (viewChange) which is simpler
...
// 4: delete show, close, error, modal, positionCenter, showMin, showMax in this class after finishing previous steps
  ...
  constructor(private inputService: InputService, private uiService: UIService,
  ...
  testValue(value: string) {
    if (this.required && !value) {
      ...
      if (this.data && Array.isArray(this.conditions) && this.conditions.length) {
        let error = false;
        for (const i in this.conditions) {
          if (!error && this.model?.[this.conditions[i]?.key] !== this.conditions[i]?.value) {
            error = true;
            this.text = this.conditions[i]?.error ? this.translatorService.translate(this.conditions[i]?.error)
              : `${this.translatorService.translate(this.conditions[i]?.key)} باید ${this.translatorService.translate(this.conditions[i]?.value)} باشد!`;
            this.valid = false;
            return {customValidation: {message: this.text}};
          }
        }
      } else if (this.regExp && !(new RegExp(this.regExp).test(value))) {
        this.valid = false;
  ...
  request($event): void {
    ...
    if (typeof this.serviceUrl === 'function' && Number($event.target?.value) > 0) {
      let fnResult = this.serviceUrl($event.target?.value);
      ...
      if (!this.pending) {
        if (typeof this.serviceUrl === 'string' && this.serviceUrl && Number($event.target?.value) > 0 && this.testValue($event.target?.value) === null) {
  ...
  setData(obj: any, value: string = null): void {
    ...
    this.model = obj?.[this.valueField]?.toString() ? obj : null;
      ...
      if (Array.isArray(textField)) {
        for (let i = 0; i < textField.length; i++) {
          if (typeof textField[i] === 'string') {
            result = obj?.[textField[i]] || obj?.[textField[i]] === 0 ? result + (i > 1 ? ' ' : '') + obj?.[textField[i]] : result;
          }
        }
