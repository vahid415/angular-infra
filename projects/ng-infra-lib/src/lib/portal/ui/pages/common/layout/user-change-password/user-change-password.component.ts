...
import { ErrorMessageType } from './../../../../../../infra/common/types/error-message';
...
export class PortalUserChangePasswordComponent implements OnInit {
  ...
  toggleEye(event) {
    if (this.changePassModel.currentPassword || this.changePassModel.newPassword || this.changePassModel.confirmPassword) {
      switch (event) {
        case 'currentPass': {
  ...
  validityInput(value, type) {
    if (!value) {
      if (type === 'current') {this.showCurrentPass = false;
      } else if (type === 'new') {this.showNewPass = false; this.changePassModel.confirmPassword = null;
